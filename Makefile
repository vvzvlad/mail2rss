...
# --- CLI (same subcommands as the service, see SPEC.md §4.5) ------------------
.PHONY: gen-secret
gen-secret: install ## Print a fresh MAIL2RSS_SECRET (128-bit base32) — back it up
	$(PY) main.py gen-secret

.PHONY: folders
folders: install ## List Fastmail folders with their mailbox ids
	$(PY) main.py folders

.PHONY: feeds
feeds: install ## Print the feed URL table (one signed URL per folder)
	$(PY) main.py feeds
